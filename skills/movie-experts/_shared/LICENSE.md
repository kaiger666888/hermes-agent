...
- `v86-pipeline-mapping.md` (Phase 27 / v5.0)
...
- **license_status:** `fair_use_paraphrase` (declared in Phase 27 `skills-mapping.yaml` `v5_ref_signoffs` section).

### `v86-pipeline-mapping.md` (Phase 27 / v5.0)

- **Source:** kais-movie-agent V8.6 SKILL.md (commit `e41fa68`, 2026-06-18 22:56:46 +0800) §"hermes-agent 专家 → 管线 Step 速查" + §"V8.6 更新" §1-§6 + §"强制审核门 (Review Gate)". Located at `/data/workspace/kais-movie-agent/SKILL.md`.
- **Copyright status:** Fair Use —
  1. V8.6 Step-to-expert mapping is factual integration architecture of the open-source kais-movie-agent project, not copyrightable expression;
  2. the 8-gate structure classification + atomic-operation annotations are original Hermes Agent analytical encoding layer;
  3. brief verbatim quotations from kais-movie-agent V8.6 update notes are explicitly attributed and within fair-use citation context (≤ 50 words per quotation).
- **Specific notes:** V8.6 13-Step names + 8-gate names are factual pipeline architecture of the upstream open-source project. Atomic-operation classification (§1-§6) paraphrased from V8.6 update notes. Per-expert V8.6 section cross-references are original Hermes Agent work.
- **license_status:** `fair_use_paraphrase` (declared in Phase 27 `skills-mapping.yaml` `v5_ref_signoffs` section).
