...
PLUGIN_NAME = "pipeline_state"
# tests/test_smoke.py -> parents[0]=tests, parents[1]=pipeline_state,
...
def test_module_imports_cleanly():
    ...
    try:
        init_mod = _load_module("_smoke_pipeline_state_init", PLUGIN_DIR / "__init__.py")
        assert hasattr(init_mod, "register"), "register() must be exported"

        tools_mod = _load_module(
            "_smoke_pipeline_state_tools", PLUGIN_DIR / "tools.py",
        )
        for symbol in (
...
def test_register_registers_4_tools_with_correct_toolset():
    """Test 3: register(ctx) registers exactly 4 tools with toolset=pipeline_state."""
    saved_path = list(sys.path)
    sys.path.insert(0, str(HERMES_ROOT))
    try:
        mod = _load_module("_smoke_pipeline_state_init_reg", PLUGIN_DIR / "__init__.py")
        ctx = _FakeCtx()
        mod.register(ctx)
    finally:
        sys.path[:] = saved_path
...
def test_handlers_return_valid_json():
    ...
    sys.path.insert(0, str(HERMES_ROOT))
    try:
        mod = _load_module("_smoke_pipeline_state_init_h", PLUGIN_DIR / "__init__.py")
    finally:
        sys.path[:] = saved_path
...
def test_python_dash_c_import_succeeds():
    """Test 5: literal ROADMAP SC#3 check — `python -c "from plugins.pipeline_state
