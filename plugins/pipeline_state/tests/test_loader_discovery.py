"""Integration tests: hermes-agent PluginManager discovers + loads ``pipeline_state``.
...
def test_enable_and_load(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert entry["tools"] == 4, (
        ...
        f"(pipeline_checkpoint_save / pipeline_checkpoint_load / "
        f"asset_bus_read / asset_bus_write), got tools={entry['tools']!r}"
    )
